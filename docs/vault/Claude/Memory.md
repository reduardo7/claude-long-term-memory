...
/memory-digest  (orchestrator, Opus model)
  ├─→ [each daily, sequential] → memory-digest-daily sub-agent
  │       └─→ extracts knowledge → docs/vault/...
  │       └─→ deletes memory/daily/<ts>.md
  │
  └─→ [each new spec, sequential] → memory-digest-spec sub-agent
...
**Model:** Opus (required — maximum extraction quality)
...
- **Memory system agents must not receive system reminders**: a hook that says "consult memory before working" must not apply to the agents that ARE the memory system. The `memory_pre_agent_reminder.py` hook excludes `memory-search`, `memory-digest-daily`, and `memory-digest-spec`.
